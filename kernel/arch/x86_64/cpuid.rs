...
const CR4_UMIP: u64 = 1 << 11; // User-Mode Instruction Prevention (SGDT/SIDT/etc. in ring-3)
...
///   - CR0.WP   : kernel write-protect (always set; no CPUID gate needed)
///   - CR4.SMEP : prevent supervisor from fetching code in user pages
///   - CR4.UMIP : block ring-3 from reading descriptor-table registers
///
/// SMAP (CR4 bit 21) is detected and logged but NOT enabled — enabling it
/// requires stac/clac guards around every kernel→user memory access.
///
/// Call once after IDT/GDT are loaded and before paging init.
pub fn init_cpu_security() {
    ...
    if has_smep {
        crate::arch::x86_64::serial::write_str(" CR4.SMEP=1");
    }
    if has_umip {
        crate::arch::x86_64::serial::write_str(" CR4.UMIP=1");
    }
    if has_smap {
        crate::arch::x86_64::serial::write_str(" (SMAP detected but not enabled: needs stac/clac guards)");
    }
    crate::arch::x86_64::serial::write_str("\r\n");
}
